...
class HierarchicalNormalMLPPolicy(Policy):
    """Policy network based on a multi-layer perceptron (MLP), with a
    `Normal` distribution output, with trainable standard deviation. This
    policy network can be used on tasks with continuous action spaces (eg.
    `HalfCheetahDir`). The code is adapted from
    ...
    This policy is a latent space policy and acts as the lower level policy.
    ...
    def forward(self, input, latent_params, params=None):
        if params is None:
            params = OrderedDict(self.named_parameters())
        output = torch.concat([input, latent_params])
        for i in range(1, self.num_layers):
            output = F.linear(output,
                weight=params['layer{0}.weight'.format(i)],
                bias=params['layer{0}.bias'.format(i)])
